...
def decaying_chirp(
    ...
    chirp_signal = np.sin(
        2 * np.pi * np.linspace(frequency_start, frequency_end, len(t)) * t
    )
...
class TestRT60(unittest.TestCase):
    ...
    def test_sanity(
        self,
        decay_rate_smaller: float,
        decay_rate_larger: float,
        fs_sig: int = 8000,
    ...
    def test_rtx_calculation(
        self, decay_rate: float = 1, fs_sig: int = 8000, fs_estimator: int = 8000
    ):
        """
        Test the basic functionality of BlindRT60.
    ...
    def test_functionality(
        ...
        e_absorption, max_order = pra.inverse_sabine(
            rt60_tgt, room_dim
        )  # Invert Sabine's formula, ISM simulator
        ...
        rt60_estimations = np.array(
            [
                blind_rt60(room.mic_array.signals[i, ...], fs)
